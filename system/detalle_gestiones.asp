...
<%
	...
	if rut="" then
		...
		if not rut="" then
			...
			strSql="INSERT INTO GESTIONES ( RUTDEUDOR,CODCLIENTE,NRODOC,CORRELATIVO,CODCATEGORIA,CODSUBCATEGORIA,CODGESTION,FECHAINGRESO,HORAINGRESO,IDUSUARIO,FECHACOMPROMISO,NRODOCPAGO,FECHAPAGO,OBSERVACIONES,CORRELATIVODATO,TELEFONO_ASOCIADO,DIRECCION_ASOCIADA,FECHA_RETIRO,FECHA_AGENDAMIENTO,HORA_AGENDAMIENTO,IDCAMPANA,ESTADO)"
			strSql= strSql & " VALUES ('"	&rut&"','"&cliente&"','1','"&correlativo&"','"&categoria&"','"&subcategoria&"','"&gestion&"','"&fecha&"','"& Mid(time,1,8) &"',"& session("session_idusuario") & "," & fechacompromiso & ",'" & comprobante & "'," & fechacancelo & ",'" & UCASE(observaciones) & "','" & Trim(correlativo2) &"','"& telges & "','" & dirges & "'," & retiro & "," & reagen & ",'" & TX_HORAAGEND & "'," & intIdCampana & ",'ABIERTO')"
...
<table align="center" width="1000" border="0">
 ...
 <table align="center" width="1000" height="740" border="0">
  <tr>
    ...
    <%
		  if not session("session_login")="" then
	        ...
	        <tr bordercolor="#999999" bgcolor="#<%=session("COLTABBG")%>" class="Estilo13">
	         ...
	         <!-- <td width="33%">GESTION</td> 29-03-2017 --> 
	        </tr>
	    ...
	    <%
	          AbrirSCG()
			  	strSql="SELECT * FROM GESTIONES_TIPO_CATEGORIA WHERE HABILITADO = 'SI'"
		...
		%>
	        ...
	        <tr>
	         <td>
	            <input name="TX_COMPROMISO" id="TX_COMPROMISO" type="text" size="10" maxlength="10" onBlur="muestra_dia();"  >
			 ...
			 <td>
	            <input name="reagen" id="reagen" type="text" id="reagen" size="10" maxlength="10" disabled >
	...
	ssql=ssql + "END as CodGestion,fechaIngreso,CONVERT(VARCHAR(10),FECHAINGRESO,108) as HORAINGRESO,IdUsuario,estado,fechacompromiso,fecha_agendamiento, horaingreso, observaciones, telefono_asociado,id_gestion "
	ssql=ssql + "from gestiones "
	ssql=ssql + "where rutdeudor= '"&rut&"' AND CODCLIENTE='" & cliente & "'"
	'ssql=ssql + "order by CONVERT(VARCHAR(10),FECHAINGRESO,108), DESC"
	ssql=ssql + "order by id_gestion desc"
     ...
     <table width="100%" border="0" bordercolor="#FFFFFF">
        <tr bordercolor="#FFFFFF" bgcolor="#<%=session("COLTABBG")%>" class="Estilo13">
		  <td class="Estilo4"><font color="yellow">TICKET</font></td>
          <td class="Estilo4"><font color="yellow">FECHA</font></td>
          <td class="Estilo4"><font color="yellow">HORA</font></td>
          <td class="Estilo4"><font color="yellow">GESTION</font></td>
          <td class="Estilo4"><font color="yellow">F.COMP.</font></td>
          <td class="Estilo4"><font color="yellow">F.AGEND</font></td>
          <td class="Estilo4"><font color="yellow">OBSERVACIONES</font></td>
           <% If TraeSiNo(session("perfil_emp")) <> "Si" Then %>
          <td WIDTH="60"class="Estilo4"><font color="yellow">FONO</font></td>
           <% End If%>
          <td class="Estilo4"><font color="yellow">USUARIO</font></td>
		  <td class="Estilo4"><font color="yellow">ESTADO</font></td>
		  <td class="Estilo4"><font color="yellow">VER</font></td>
		  <td class="Estilo4"><font color="yellow">ACCION</font></td>
          </tr>
		<%do until rsDET.eof
		...
		strCodCat= rsDET("CodCategoria")
		strSubCat= rsDET("CodSubCategoria")


		strSql = "SELECT Descripcion from Gestiones_Tipo_Categoria where CodCategoria = '" & strCodCat & "'"
		strSql2 = "SELECT Descripcion from Gestiones_Tipo_SubCategoria where CodCategoria = '" & strCodCat & "' and CodSubCategoria = '" & strSubCat & "'"
		...
		If Not rsNomGestion.Eof Then
			strNomGestion = rsNomGestion("Descripcion")
		Else
			strNomGestion = ""
		End If

		SET rsNomSubGestion=Conn.execute(strSql2)
		If Not rsNomSubGestion.Eof Then
			strNomSubGestion = rsNomSubGestion("Descripcion")
		Else
			strNomSubGestion = ""
		End If
        ...
        <tr bordercolor="#FFFFFF" class="Estilo8">
		  <td class="Estilo4"><%=rsDET("id_gestion")%></td>
          <td class="Estilo4"><%=rsDET("FechaIngreso")%></td>
          <td class="Estilo4"><%=rsDET("HORAINGRESO")%></td>
          <td class="Estilo4"><%=strNomGestion%>-<%=strNomSubGestion%></td>
		  ...
		  <td class="Estilo4"><div style="font:bold;color:red;align:center"><%=rsDET("Estado")%></div></td>
	<!--	  <td class="Estilo4"><a href="cambia_estado.asp?gestion=<%=rsDET("id_gestion")%>&deudor=<%=rut%>">CAMBIAR</a></td> -->
		  <td><div align="center"><a href="ver_detalles.asp?gestion=<%=rsDET("id_gestion")%>"><img src="search.png" border="0" alt='' width="20px" height="20px" /></a></div>
		  	&nbsp;
		  <%
		  If session("session_idusuario") = "503" or session("session_idusuario") = "307" Then
		  %>
		  <td><div align="center"><a href="eliminar.asp?gestion=<%=rsDET("id_gestion")%>"><img src="delete-button.png" border="0" alt='' width="20px" height="20px" /></a></div></td>
		  <%
		  End IF
		  %>	
...
function GestionesCompPago() {
	...
	for ( i=0; i < x.length; i++ ) {
		...
		if (datos.cmbsubcat.value != '4') {
			return(false);
			break;
		}
		else if ((datos.cmbcat.value + '-' + datos.cmbsubcat.value + '-') && (datos.TX_COMPROMISO.value == '')) {
			return(true);
			break;
		}
	}
...
function GestionesAgendables() {
	...
	//alert(x[i]);
		...
		else if ((datos.cmbcat.value + '-' + datos.cmbsubcat.value + '-') && (datos.reagen.value == '')) {
			return(true);
			break;
		}
	}
...
function nueva(){
	...
	if((datos.cmbcat.value=='0')||(datos.cmbsubcat.value=='0')){
		...
		document.getElementById("TX_COMPROMISO").focus();
	}else if (GestionesAgendables()){
		alert('DEBE INGRESAR FECHA DE REAGENDAMIENTO');
		document.getElementById("reagen").focus();
...
//		alert('DEBE INGRESAR UNA GESTION');
		...
		document.getElementById("telges").focus();
