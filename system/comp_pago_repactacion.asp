...
<table width="600" border="1" bordercolor = "#000000" cellSpacing=0 cellPadding=1>
	...
	<%
		strSql = "SELECT NOMBRE_B , NRO_CTACTE , NRO_CHEQUE, MONTO, VENCIMIENTO , FORMA_PAGO FROM CAJA_WEB_EMP_DOC_PAGO, BANCOS WHERE BANCOS.CODIGO = CAJA_WEB_EMP_DOC_PAGO.COD_BANCO AND ID_PAGO = " & rsCaja("ID_PAGO") & " ORDER BY FORMA_PAGO, VENCIMIENTO, NRO_CTACTE, NRO_CHEQUE"
...
<%
				...
				strSql = "SELECT COUNT(*) as CANTC FROM REPACTACION_DET WHERE CUOTA <> 0 AND ID_CONVENIO = " & intIdConvenio
				...
				If Not rsDetCaja.eof Then
					...
					Do While not rsDetCaja.Eof
						...
						strSql="SELECT CUOTA, FECHA_PAGO FROM REPACTACION_DET WHERE CUOTA <> 0 AND ID_CONVENIO = " & intIdConvenio & " AND CUOTA = " & rsDetCaja("NRODOC")
