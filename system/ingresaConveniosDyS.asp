...
ssql=ssql +"values('"+rut+"','36','1',dbo.CORRELATIVO_GESTION_CLIENTE ('"+rut+"','36'),1,2,8,getdate(),CONVERT(VARCHAR(20),GETDATE(),108),'" + cobrador + "','"+fecha+"','REALIZO CONVENIO DE PAGO','"+Fono+"',1)"
'response.Write(ssql)
Conn.execute(ssql)


SSQL2=SSQL2 + "insert into ConsolidadosConveniosDys "
SSQL2=SSQL2 + "(codcobrador,rutdeudor,telefono,fechagestion,fechaconvenida,nrocuotas,valorcuotas,total,folio,producto,REPETICION) "
SSQL2=SSQL2 + "values "
SSQL2=SSQL2 + "('" + cobrador + "','"+rut+"',RTRIM(REPLACE('"+Fono+"','-','')),  GETDATE(), '"+fecha+"', "+cuotas+","+montocuota+","+total+",'"+folio+"','"+producto+"','" + REPETICION +"')"
...
sql3="	select id,DATEDIFF ( second ,  rectime ,getdate()) "
sql3=sql3 & "FROM RECORDERMSG "
sql3=sql3 & "WHERE DATEPART(YEAR,RECTIME)= " & aa
sql3=sql3 & " AND DATEPART(MONTH,RECTIME)=" & mm
sql3=sql3 & " AND DATEPART(DAY,RECTIME)= " & dd
sql3=sql3 & " AND RTRIM(LABEL)  <>''"
sql3=sql3 & " and nroremoto like '%'+RTRIM(REPLACE('"+Fono+"','-',''))"
sql3=sql3 & "and DATEDIFF ( second ,  rectime ,getdate())=(select min ( DATEDIFF ( second ,  rectime ,getdate()))FROM RECORDERMSG "
sql3=sql3 & "WHERE DATEPART(YEAR,RECTIME)= " & aa
sql3=sql3 & " AND DATEPART(MONTH,RECTIME)= " & mm
sql3=sql3 & " AND DATEPART(DAY,RECTIME)= " & dd
sql3=sql3 & " AND RTRIM(LABEL)  <>'' "
...
set rsCLI=ConnD.execute(sql3)
	if ( not rsCLI.eof) then
		'response.write(rsCLI("id"))
		...
		SSQL3=SSQL3 + " AND NROREMOTO LIKE '%'+RTRIM(REPLACE('"+Fono+"','-',''))  and id="& rsCLI("id")
...
<TABLE>
	<TR><td>USUARIO  </td>
      <td> <strong><%=session("session_login")%></strong></td>
    </TR>
	...
	<TR>
      <td>ES REPETICION</td>
      <td><select name="REPETICION" size="1">
