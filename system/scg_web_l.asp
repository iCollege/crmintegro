      ...
      <table width="793" border="0" align="center" cellpadding="0" cellspacing="0">
        ...
        <tr>
          ...
          <td width="758">
          	<table width="100%" height="431" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF"><script type="text/javascript" language="JavaScript1.2" src="frameset/stm31.js"></script>
          		<tr valign=TOP>
							<td height="431" colspan="3"><p align="right">Usuario: <%=UCASE(session("nombre_user"))%> | <a href="javascript:ventanaMisGestiones('mis_gestiones.asp?codcob=<%=LTRIM(RTRIM(session("session_login")))%>')"><acronym title="GESTIONES PRODUCIDAS POR <%=session("nombre_user")%>">MIS GESTIONES</acronym></a> | MI RECUPERACI&Oacute;N |
		...
		if not rsDEU.eof then
			...
			rut_deudor = rsDEU("RUTDEUDOR")
					...
					sORA = "SELECT TOP 1 NOMBRE,RUT,DIGV FROM  T_JUICIO WHERE RUT = '"&rut_ora&"'"
	  ...
	  %>
	   ...
	   <tr>
		   ...
		   <td>
				...
				<td><div align="right">$ <%=(FN(CLNG(rsDEU("MONTODOC")),0))%></div>
		   </td>
		   <td>
              ...
              <%
		 		...
		 		%>
					...
					<tr>
						...
						<td>
							<a href="javascript:ventanaIngresoG('detalle_gestiones.asp?rut=<%=rut_deudor%>&cliente=7')">MODULO DE GESTION </a>
						</td>
						<td>
							<div align="right"><%=rsMO("CANTIDAD")%></div>
						</td>
						<td>
							<div align="right">$ <%=FN(CLNG(monto),0)%></div>
						</td>
						<td>
							<div align="right"><a href="javascript:ventanaDetalleINPP('detalle_deuda_inp.asp?rut=<%=rut_deudor%>&cliente=7')">VER DETALLE DEUDA </a></div>
						</td>
					    <td>&nbsp;</td>
					</tr>
					<%end if
