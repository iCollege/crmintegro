...
<table width="600" height="420" border="0">
  <tr>
    <td height="20"><img src="../lib/DETALLE_DEUDA.gif" width="600" height="22"></td>
  </tr>
  <tr>
    <td height="20"><img src="../lib/41.gif" width="600" height="22"></td>
  </tr>
  <tr>
    ...
    abrirscg()
	ssql=""
	ssql="SELECT ETAPACOBRANZA,NRODOC,VALORCUOTA,SALDO,ISNULL(MONTOEMPRESA,0) AS MONTOEMPRESA,CODCOBRADOR,NROCUOTA FROM CUOTA WHERE RUTDEUDOR='"&rut&"' AND CODCLIENTE='"&cliente&"'"
	...
	<table width="600" border="0" bordercolor="#FFFFFF">
        ...
        <tr bordercolor="#999999" >
          ...
          <td>
		  <%
		  sql_cuenta = "SELECT TOP 1 numero_doc_oficial,numero_cuenta,interlocutor,fecha_facturacion FROM ACSA_A1_DETALLE_INFRA WHERE numero_documento='"&rsDET("NRODOC")&"'"
		  set rsMAS=Conn.execute(sql_cuenta)
			  if not rsMAS.eof then
			  docoficial=rsMAS("numero_doc_oficial")
			  nrocuenta=rsMAS("numero_cuenta")
			  interlocutor=rsMAS("interlocutor")
			  ffactura = rsMAS("fecha_facturacion")
			  end if
		  rsMAS.close
		  set rsMAS=Nothing
          ...
          <td><%=ffactura%></td>
          <td><div align="left" ><%=nrocuenta%></div></td>
      ...
      <%
	  ssql_pat = "SELECT DISTINCT patente,tag,cta_contrato FROM ACSA_A3_DETALLE_INFRA WHERE RUT_DEUDOR ='"&rut&"' order by cta_contrato"
