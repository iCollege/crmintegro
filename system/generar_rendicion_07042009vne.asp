...
<title>ATO</title>
		...
		strSql = strSql & " AND FECHA_PAGO >= '" & inicio & " 00:00" & "' AND FECHA_PAGO <= '" & termino & " 23:59" & "' AND TIPO_PAGO NOT IN ( 'CO','PH') "
		...
		If Not rsCaja.eof Then
			...
			strCabTotalSaldo = 0
				...
				Do While not rsCaja.Eof
					...
					strSql = "SELECT MIN(NRODOC) AS NRODOC, CUENTA, SUM(CAPITAL) AS CAPITAL"
					strSql = strSql & " FROM CAJA_WEB_EMP_DETALLE WHERE ID_PAGO = " & rsCaja("ID_PAGO") & " GROUP BY CUENTA"
