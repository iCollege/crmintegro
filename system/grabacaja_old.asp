...
<%
	...
	if sw = 1 and sw2=1 and sw3=1 and sw4=1 then
		strSql = "EXEC UPD_SEC 'CAJA_WEB_CPS'"
					...
					strSql = strSql & "VALUES (" & intSeq & ",'" & fecha & "','" & fecha & "',YEAR(getdate()),'" & intSucursal & "','" & strCodCliente & "','" &  strRutDeudor & "'," & intCompIngreso  & "," & intNroBoleta & "," & intMontoCliente & "," & intMontoCps & ",'" & intTipoPago & "','" & strNroDepositoCliente & "','" & strNroDepositoCPS & "'," & usuario & "," & strDescCliente & "," & intDescuento & "," & strTotalCliente & "," & strTotalCps & ",'" & bancocliente & "','" & bancocps & "','" & observaciones & "'," & ipcliente & "," & ccps & ",'" & estado & "')"
		...
		'response.end
					...
					<%
					Response.Write ("<script language = ""Javascript"">" & vbCrlf)
					Response.Write (vbTab & "location.href='caja_web2_old.asp?rut=" & rut & "&tipo=1'" & vbCrlf)
					Response.Write ("</script>")
	end if
