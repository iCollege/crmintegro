...
<%
	...
	if rut="" then
		...
		if not rut="" then
			...
			strSql="INSERT INTO GESTIONES ( RUTDEUDOR,PATENTE,CODCLIENTE,NRODOC,CORRELATIVO,CODCATEGORIA,CODSUBCATEGORIA,CODGESTION,FECHAINGRESO,HORAINGRESO,IDUSUARIO,FECHACOMPROMISO,NRODOCPAGO,FECHAPAGO,OBSERVACIONES,CORRELATIVODATO,TELEFONO_ASOCIADO,DIRECCION_ASOCIADA,FECHA_RETIRO,FECHA_AGENDAMIENTO,HORA_AGENDAMIENTO,IDCAMPANA,ESTADO)"
			strSql= strSql & " VALUES ('"&rut&"','"&patente&"','"&cliente&"','1','"&correlativo&"','"&categoria&"','"&subcategoria&"','"&gestion&"',getdate(),'"& Mid(time,1,8) &"',"& session("session_idusuario") & "," & fechacompromiso & ",'" & comprobante & "'," & fechacancelo & ",'" & UCASE(observaciones) & "','" & Trim(correlativo2) &"','"& telges & "','" & dirges & "'," & retiro & "," & reagen & ",'" & TX_HORAAGEND & "'," & intIdCampana & ",'ABIERTO')"
...
<table align="center" width="980" border="0">
 ...
 <table align="center" width="980" height="740" border="0">
  <tr>
    ...
    <%
		...
		%>
	        ...
	        <tr bgcolor="#<%=session("COLTABBG")%>" class="Estilo13" >
			  <td><font color="yellow">PATENTE</font></td>
	        ...
	        <tr>
			 <td>
	            <input name="TX_PATENTE" id="TX_PATENTE" autofocus type="text" size="6" maxlength="6">
			 </td>
	         <td>
	...
	'ssql=ssql + "order by id_gestion desc"
     ...
     <table width="100%" border="0" bordercolor="#FFFFFF">
        <tr bordercolor="#FFFFFF" bgcolor="#<%=session("COLTABBG")%>" class="Estilo13">
		  ...
		  <td class="Estilo4"><font color="yellow">PATENTE</font></td>
