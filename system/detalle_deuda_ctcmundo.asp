...
<table width="680" height="420" border="0">
  ...
  <tr>
       <td height="20"><img src="../lib/13.gif" width="740" height="22"></td>
  </tr>
  <tr>
	...
	<table width="100%" border="0" bordercolor="#FFFFFF">
        <tr bordercolor="#999999" bgcolor="#<%=session("COLTABBG")%>" class="Estilo13">
          <td width="15%"><div align="left">N&ordm; DOCUMENTO </div></td>
          <td width="15%"><div align="left">EMISI&Oacute;N</div></td>
          <td width="15%"><div align="left">VENCIMIENTO</div></td>
          <td width="15%"><div align="left">MONTO</div></td>
          <td width="15%"><div align="left">SALDO</div></td>
        </tr>
		<%do until rsDET.eof
		%>
		...
		total_docs = total_docs + 1
	 	...
	 	<table width="100%" border="0">
          <tr>
            ...
            <td width="24%" bgcolor="#<%=session("COLTABBG")%>"><span class="Estilo27">TOTALES</span></td>
            <td width="20%" bgcolor="#<%=session("COLTABBG2")%>"><div align="right"><span class="Estilo28">$ <%=FN(total,0)%></span></div></td>
            <td width="20%" bgcolor="#<%=session("COLTABBG2")%>"><div align="right"><span class="Estilo28">$ <%=FN(total_saldo,0)%></span></div></td>
          </tr>
        </table>
	 	<br><br>
	 	<strong>TELEFONOS ASOCIADOS A LA DEUDA :</strong>	 	<%
		ssql44="SELECT codarea,fono FROM CTC_MUNDO_FONO WHERE rut_deudor='"&rut&"'"
		set rsFC=Conn.execute(ssql44)
		do until rsFC.eof
			response.Write("  ")
			response.Write( rsFC("codarea") )
			response.Write("-")
			response.Write(rsFC("fono"))%>
			&nbsp;&nbsp;&nbsp;
<%
		rsFC.movenext
		loop
