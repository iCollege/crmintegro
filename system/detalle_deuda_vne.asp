...
<table width="676" height="420" border="0">
  ...
  <tr>
    <td height="20" class="Estilo20"><img src="../lib/<%=cliente%>.gif" width="600" height="22"></td>
  </tr>
  <tr>
	...
	%>
	 ...
	 <% 'abrirscg()
		ssql= "select c.referencia,c.nrodoc,c.fechavenc,c.interlocutor,c.ctactrato,c.tipocliente,c.tipodoc,sum(c.valorcuota) as deuda "
		ssql=ssql + " from cuota_vneprev c, rut_interloccom_vne r "
		ssql=ssql + "where r.rut='" & rut & "' "
		ssql=ssql + "and c.interlocutor=r.interloccom "
		ssql=ssql + "and c.fechavenc>DATEADD (day,-5,getdate()) "
		ssql=ssql + "group by c.referencia,c.nrodoc,c.fechavenc,c.interlocutor,c.ctactrato,c.tipocliente,c.tipodoc "
		ssql=ssql + "order by c.fechavenc"

		set rsDET=Conn.execute(ssql)
		if not rsDET.eof then %>
	  <!--  ver si tiene deudas por vencer sino no se muestra lo que sigue				-->

	  <table width="670" border="0" bordercolor="#FFFFFF">
	  <tr bordercolor="yellow" bgcolor="#FFFF00" class="Estilo13">
          <td width="100%"> <font color="#000000">DOCUMENTOS POR VENCER</FONT></td>
      </tr>
	  </TABLE>
	  <table width="671" border="0" bordercolor="#FFFFFF">
        <tr bordercolor="#999999" bgcolor="#<%=session("COLTABBG")%>" class="Estilo13">
          <td width="14%">DOCUMENTO</td>
          <td width="10%">CTACTRATO</td>
          <td width="10%">FECHA VENC</td>
          <td width="11%">TIPODOC</td>
          <td width="11%">SALDO</td>
		...
		do until rsDET.eof%>
	          <tr bordercolor="#999999" >
    	        <td>N&ordm; <%=rsDET("NRODOC")%></td>
        	    <td><%=rsDET("ctactrato") %></td>
				<td><div align="left" >  <%=FormatDateTime(rsDET("FECHAVENC"),2)%></div></td>
          		<td><div align="right"><%=rsDET("tipodoc")%></div></td>
          		<td><div align="right">$ <%=FN(rsDET("deuda"),0)%></div></td>
		 	  </tr>
		 	<%total_docs2=total_docs2+1
			total_saldo2=total_sald2o+FN(rsDET("deuda"),0)
				rsDET.movenext
		 loop
		 %>
		<tr>
          <td height="33" bgcolor="#<%=session("COLTABBG")%>"><span class="Estilo27">DOCUMENTOS:&nbsp;<%=total_docs%></span></td>
          <td bgcolor="#<%=session("COLTABBG")%>"></td>
          <td bgcolor="#<%=session("COLTABBG")%>" colspan="1"><div align="left"><span class="Estilo27"></span></div></td>
		  <td bgcolor="#<%=session("COLTABBG")%>"><span class="Estilo27">TOTALES</span></td>
          <td bgcolor="#<%=session("COLTABBG2")%>"><div align="right"><span class="Estilo28">$
              <%=FN(total_saldo2,0)%></span></div></td>
