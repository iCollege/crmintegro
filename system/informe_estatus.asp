...
If trim(intRemesa) <> "" Then
	strSqlRemesa = " AND C.CODREMESA = " & intRemesa
End if
...
If Trim(strCliente) = "" Then cliente = "1000"
...
<table width="850" border="0">
  <tr>
      ...
      <tr>
        ...
        <td>
			<select name="CB_PROCURADOR">
				...
				<%
				abrirscg()
					ssql="SELECT ID_USUARIO,LOGIN FROM USUARIO WHERE PERFIL_PROC = 1"
					set rsTemp= Conn.execute(ssql)
					if not rsTemp.eof then
						do until rsTemp.eof%>
						<option value="<%=rsTemp("ID_USUARIO")%>"<%if cint(intCobrador)=rsTemp("ID_USUARIO") then response.Write("Selected") End If%>><%=rsTemp("LOGIN")%></option>
    ...
    <%
		...
		If Trim(intGestion) <> "T" AND Trim(intGestion) <> "R" Then
			'strSql="SELECT MIN(FECHAVENC) as MFVENC, R.FECHA_ASIGNACION, C.RUTDEUDOR, '' as NUM_CLIENTE, IsNull(SUM(C.SALDO),0) as SALDO, COUNT(C.NRODOC) as CNRODOC , C.TIPOCOB, IsNull(C.IDDEMANDA,0) as IDDEMANDA FROM CUOTA C, REMESA R WHERE C.CODREMESA = R.CODREMESA AND C.CODCLIENTE = R.CODCLIENTE AND C.CODCLIENTE = '" & strCliente & "' AND IDGJUDICIAL = " & intGestion & " AND C.ESTADO_DEUDA = 1 " & strCondicion & strSqlUsuario & strSqlRemesa & " GROUP BY R.FECHA_ASIGNACION, C.RUTDEUDOR, C.TIPOCOB, C.IDDEMANDA ORDER BY SALDO DESC"
			strSql="SELECT MIN(FECHAVENC) as MFVENC, R.FECHA_ASIGNACION, C.RUTDEUDOR, '' as NUM_CLIENTE, IsNull(SUM(C.SALDO),0) as SALDO, COUNT(C.NRODOC) as CNRODOC , C.TIPOCOB, Max(IsNull(C.IDDEMANDA,0)) as IDDEMANDA FROM CUOTA C, REMESA R WHERE C.CODREMESA = R.CODREMESA AND C.CODCLIENTE = R.CODCLIENTE AND C.CODCLIENTE = '" & strCliente & "' AND IDGJUDICIAL = " & intGestion & " AND C.ESTADO_DEUDA = 1 " & strCondicion & strSqlUsuario & strSqlRemesa & " GROUP BY R.FECHA_ASIGNACION, C.RUTDEUDOR, C.TIPOCOB ORDER BY SALDO DESC"
		Else
			If Trim(intGestion) = "T" Then
				'strSql="SELECT MIN(FECHAVENC) as MFVENC, R.FECHA_ASIGNACION, C.RUTDEUDOR, '' as NUM_CLIENTE, IsNull(SUM(C.SALDO),0) as SALDO, COUNT(C.NRODOC) as CNRODOC, C.TIPOCOB, IsNull(C.IDDEMANDA,0) as IDDEMANDA FROM CUOTA C, REMESA R WHERE C.CODREMESA = R.CODREMESA AND C.CODCLIENTE = R.CODCLIENTE AND C.CODCLIENTE = '" & strCliente & "' AND C.ESTADO_DEUDA = 1 " & strCondicion & strSqlUsuario & strSqlRemesa & " GROUP BY R.FECHA_ASIGNACION, C.RUTDEUDOR, C.TIPOCOB, C.IDDEMANDA ORDER BY SALDO DESC"
				strSql="SELECT MIN(FECHAVENC) as MFVENC, R.FECHA_ASIGNACION, C.RUTDEUDOR, '' as NUM_CLIENTE, IsNull(SUM(C.SALDO),0) as SALDO, COUNT(C.NRODOC) as CNRODOC, C.TIPOCOB, Max(IsNull(C.IDDEMANDA,0)) as IDDEMANDA FROM CUOTA C, REMESA R WHERE C.CODREMESA = R.CODREMESA AND C.CODCLIENTE = R.CODCLIENTE AND C.CODCLIENTE = '" & strCliente & "' AND C.ESTADO_DEUDA = 1 " & strCondicion & strSqlUsuario & strSqlRemesa & " GROUP BY R.FECHA_ASIGNACION, C.RUTDEUDOR, C.TIPOCOB ORDER BY SALDO DESC"
			End If
			If Trim(intGestion) = "R" Then
				'strSql="SELECT MIN(FECHAVENC) as MFVENC, R.FECHA_ASIGNACION, C.RUTDEUDOR, '' as NUM_CLIENTE, IsNull(SUM(C.SALDO),0) as SALDO, COUNT(C.NRODOC) as CNRODOC, C.TIPOCOB, IsNull(C.IDDEMANDA,0) as IDDEMANDA FROM CUOTA C, REMESA R WHERE C.CODREMESA = R.CODREMESA AND C.CODCLIENTE = R.CODCLIENTE AND C.CODCLIENTE = '" & strCliente & "' AND C.ESTADO_DEUDA = 2 " & strCondicion & strSqlUsuario & strSqlRemesa & " GROUP BY R.FECHA_ASIGNACION, C.RUTDEUDOR, C.TIPOCOB, C.IDDEMANDA ORDER BY SALDO DESC"
				strSql="SELECT MIN(FECHAVENC) as MFVENC, R.FECHA_ASIGNACION, C.RUTDEUDOR, '' as NUM_CLIENTE, IsNull(SUM(C.SALDO),0) as SALDO, COUNT(C.NRODOC) as CNRODOC, C.TIPOCOB, Max(IsNull(C.IDDEMANDA,0)) as IDDEMANDA FROM CUOTA C, REMESA R WHERE C.CODREMESA = R.CODREMESA AND C.CODCLIENTE = R.CODCLIENTE AND C.CODCLIENTE = '" & strCliente & "' AND C.ESTADO_DEUDA = 2 " & strCondicion & strSqlUsuario & strSqlRemesa & " GROUP BY R.FECHA_ASIGNACION, C.RUTDEUDOR, C.TIPOCOB ORDER BY SALDO DESC"
			End If
		End If
		'Response.write "strSql = " & strSql
		'Response.End
		...
		<tr bgcolor="#<%=session("COLTABBG")%>" class="Estilo13">
				...
				<% End if %>
				<% If Trim(intGestion)= "2" or Trim(intGestion)= "3" or Trim(intGestion)= "4" or Trim(intGestion)= "14" or Trim(intGestion)= "15" Then %>
   	...
   	<%
		...
		''Response.End
			Do until rsGTC.eof
				...
				strFechaVenc= rsGTC("MFVENC")
				strCantDoc = rsGTC("CNRODOC")
				...
				'strSql="SELECT PROB_COBRO, HONORARIOS FROM CUOTA_ENC WHERE RUTDEUDOR='" & strRutDeudor &"' AND CODCLIENTE = " & strCliente
				...
				If not rsDemanda.eof Then
					...
					strProcurador = TraeCampoId(Conn, "NOMBRES_USUARIO", rsDemanda("IDPROCURADOR"), "USUARIO", "ID_USUARIO") & " " & TraeCampoId(Conn, "APELLIDOS_USUARIO", rsDemanda("IDPROCURADOR"), "USUARIO", "ID_USUARIO")
				...
				%>
					...
					<td><%=strTelefono%></td>
					<td>
					<a href="javascript:ventanaDetalle('detalle_deuda.asp?rut=<%=strRutDeudor%>&cliente=<%=strCliente%>')">
					Detalle
					</a>
					</td>
				<% End if %>
...
window.open(URL,"DETALLEGESTION","width=850, height=300, scrollbars=yes, menubar=no, location=no, resizable=yes")
}
