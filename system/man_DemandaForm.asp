...
<%
	...
	If Not srsRegistro.Eof Then
		...
		strTipoDda=Trim(srsRegistro("TIPO_DEMANDA"))
...
<table width="100%" border="0" cellpadding=2 cellspacing=0>
 ...
 <table width="100%" border="0" CLASS="tabla1">
    	<TR BGCOLOR="#FFFFFF">
    		<td class="hdr_i"><img src='../images/boton_no_1.gif' align='absmiddle'>&nbsp;INGRESO DEMANDA</Font></td>
    		<td class="hdr_i">
    			<A HREF="visor_documentos.asp?strRut=<%=strRut%>">
					Ver Documentacion
				</A>
    		</td>
...
<table width="100%" border="0" CLASS="tabla1">
	...
	<% If Trim(strTipoDda) = "" Then strSelTipoDdaSel="SELECTED" %>
	<% If Trim(strTipoDda) = "FULL" Then strSelTipoDdaFull="SELECTED" %>
	<% If Trim(strTipoDda) = "LIGHT" Then strSelTipoDdaLight="SELECTED" %>
	<% If Trim(strTipoDda) = "TIMBRE" Then strSelTipoDdaTimbre="SELECTED" %>


	<TR BGCOLOR="#FFFFFF">
		<td class="hdr_i">Tipo Demanda</Font></td>
		<td class="td_t">
			<SELECT NAME="CB_TIPO_DDA">
				<OPTION VALUE="0" <%=strSelTipoDdaSel%>>SELECCIONE</OPTION>
				<OPTION VALUE="FULL" <%=strSelTipoDdaFull%>>Full</OPTION>
				<OPTION VALUE="LIGHT" <%=strSelTipoDdaLight%>>Light</OPTION>
				<OPTION VALUE="TIMBRE" <%=strSelTipoDdaTimbre%>>Light</OPTION>
			</SELECT>
		</td>
	</tr>
	...
	<TR BGCOLOR="#FFFFFF">
		<td class="hdr_i">Fecha Presentación Escrito</Font></td>
		<td class="td_t"><% general_MostrarCampo "FECHA_PRES_ESCRITO", False, Null, Null,srsRegistro %></td>
	</TR>
	...
	<TR BGCOLOR="#FFFFFF">
		...
		<td class="td_t"><% general_MostrarCampo "FECHA_COMPARENDO", False, Null, Null,srsRegistro %>
		&nbsp&nbsp&nbsp&nbspHora : &nbsp&nbsp<% general_MostrarCampo "HORA_COMPARENDO", False, Null, Null,srsRegistro %>
		</td>
...
<table width="100%" border="0" CLASS="tabla1">
	<TR>
		<td class="hdr_i" width="50%">
			Notificaciones Ingresadas
		</TD>
		<td class="hdr_i">

				<A HREF="man_notificacionForm.asp?sintNuevo=1&IDDEMANDA=<%=IntId%>">
					Nueva Notificacion
				</A>
		</TD>
    </TR>
	...
	<TR>
		<TD ALIGN=LEFT colspan=2>
			...
			<TR BGCOLOR="#F3F3F3">
				...
				<TD><b>Tipo</TD>
				<TD WIDTH="60"><b>Valor</TD>
				<TD>&nbsp</TD>
			</TR>

			<%
				strSql="SELECT DESCRIPCION, IDNOTIFICACION , CONVERT(VARCHAR(10),FECHA,103) AS FECHA, IDESTADONOTIF, VALOR FROM DEMANDA_NOTIF N, ESTADO_NOTIFICACION T WHERE T.CODIGO = N.IDESTADONOTIF AND N.IDDEMANDA = " & IntId
				set rsNotificacion=Conn.execute(strSql)
				...
				Do until rsNotificacion.eof
					...
					<TR BGCOLOR="#FFFFFF">
						...
						<td class="td_t"><%=rsNotificacion("DESCRIPCION")%></td>
						<td class="td_t"><%=rsNotificacion("VALOR")%></td>
						<td class="td_t">
						<A HREF="man_notificacionForm.asp?sintNuevo=0&IDDEMANDA=<%=IntId%>&IDNOTIFICACION=<%=rsNotificacion("IDNOTIFICACION")%>">
							Ver Notificacion
						</A>
						</td>
					</TR>
