...
{
	resp='no'
	datos.action = "repactaciones_vencidas.asp?resp="+ resp +"";
	datos.submit();
}
...
{
	resp='si'
	datos.action = "repactaciones_vencidas.asp?strAgrupado=S&resp="+ resp +"";
	datos.submit();
}
...
<table width="850" height="500" border="1">
  <tr>
    <td height="20" bordercolor="#999999"  bgcolor="#<%=session("COLTABBG")%>" class="Estilo13" ALIGN="CENTER">LISTADO DE REPACTACIONES VENCIDAS</td>
  </tr>
  <tr>
    <td valign="top">
	...
	<% If resp="si" then
			...
			If trim(strAgrupado)="S" Then
				strSql = "SELECT D.ID_CONVENIO, COUNT(*) AS CUOTA, sum(TOTAL_CUOTA) AS TOTAL_CUOTA, CONVERT(VARCHAR(10),min(FECHA_PAGO),103) as FECHA_PAGO, E.COD_CLIENTE,E.RUTDEUDOR , IsNull(datediff(d,mIN(FECHA_PAGO),getdate()),0) as ANTIGUEDAD FROM REPACTACION_DET D, REPACTACION_ENC E WHERE D.ID_CONVENIO = E.ID_CONVENIO AND D.FECHA_PAGO < '" & inicio & "' AND D.PAGADA IS NULL"
			Else
				...
				strSql = strSql & " FROM REPACTACION_DET D, REPACTACION_ENC E WHERE D.ID_CONVENIO = E.ID_CONVENIO "
				strSql = strSql & " AND D.FECHA_PAGO < '" & inicio & "' AND D.PAGADA IS NULL"
			End If
	...
	if strSql <> "" then
		...
		if not rsDet.eof then
			...
			<tr bgcolor="#<%=session("COLTABBG2")%>" class="Estilo8">
				...
				<td><A HREF="det_repactacion.asp?id_convenio=<%=rsDet("ID_CONVENIO")%>"><acronym title="VER REPACTACION">Ver </acronym></A>&nbsp;<%=rsDet("ID_CONVENIO")%></td>
