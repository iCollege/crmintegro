...
<table width="680" height="420" border="0">
  ...
  <tr>
    ...
    abrirscg()
	ssql=""
	ssql="SELECT seriedecimal,NRODOC,VALORCUOTA,SALDO,GASTOSCOBRANZAS,FECHAEMISION,TIPOPRODUCTO,FECHAVENC,CODCOBRADOR,INTERESES,NROCUOTA,GLOSA,datediff(day,fechaemision,getdate()) AS DIASMORA FROM CUOTA WHERE RUTDEUDOR='"&rut&"' AND CODCLIENTE='"&cliente&"'"
	...
	IF RSDET("GLOSA")="N" OR RSDET("GLOSA")="S" THEN
		GLOSA="CARTERA NORMAL"
	ELSEIF RSDET("GLOSA")="N              A" OR RSDET("GLOSA")="S              A" THEN
		GLOSA="CARTERA RENEGOCIADA DURA"
	ELSEIF RSDET("GLOSA")="S              N" OR RSDET("GLOSA")="N              N" THEN
	...
	'	camp_especial=""
	'ELSE
	IF trim(RSDET("seriedecimal"))="0"  THEN
		camp_especial="Campaña Especial Tasa 0%"
	ELSEIF trim(RSDET("seriedecimal"))="3,715" THEN
'		RESPONSE.Write(RSDET("seriedecimal") )
'		RESPONSE.End()
		camp_especial="Campaña Especial Tasa 3,715%"
	else
		response.Write(Trim(rsdet("seriedecimal")))
		camp_especial=""
	END IF
