...
<table width="840" border="1" bordercolor="#999999" cellpadding="2" cellspacing="5">
  ...
  <tr>
	...
	strSql="SELECT IsNull(ADIC1,'ADIC1') as ADIC1, IsNull(ADIC2,'ADIC2') as ADIC2, IsNull(ADIC3,'ADIC3') as ADIC3, IsNull(ADIC4,'ADIC4') as ADIC4, IsNull(ADIC5,'ADIC5') as ADIC5, IsNull(ADIC91,'ADIC91') as ADIC91, IsNull(ADIC92,'ADIC92') as ADIC92, IsNull(ADIC93,'ADIC93') as ADIC93, IsNull(ADIC94,'ADIC94') as ADIC94, IsNull(ADIC95,'ADIC95') as ADIC95, USA_CUSTODIO, IsNull(COLOR_CUSTODIO,'FFFFFF') as COLOR_CUSTODIO, INTERES_MORA, COD_TIPODOCUMENTO_HON FROM CLIENTE WHERE CODCLIENTE = '" & strCodCliente & "'"
...
<table width="840" border="1" bordercolor="#999999" cellpadding="2" cellspacing="5">
	...
	<table width="840" border="0" ALIGN="CENTER">
	  <tr>
		...
		If Trim(rut) <> "" then
		abrirscg()
			strSql="SELECT IDCUOTA, DATEDIFF(MONTH,FECHAVENC,GETDATE()) AS ANT_MESES, RUTDEUDOR, IsNull(FECHAVENC,'01/01/1900') as FECHAVENC, IsNull(datediff(d,fechavenc,getdate()),0) as ANTIGUEDAD, NRODOC,IsNull(VIGENTE,0) as VIGENTE, IsNull(VALORCUOTA,0) as VALORCUOTA,IsNull(SALDO,0) as SALDO,isnull(GASTOSPROTESTOS,0) as GASTOSPROTESTOS,IsNull(USUARIO_ASIG,0) as USUARIO_ASIG,NROCUOTA, VIGENTE, IsNull(MORA,0) as MORA, IsNull(VENCIDA,0) as VENCIDA, IsNull(CASTIGO,0) as CASTIGO, CENTRO_COSTO, SUCURSAL , ESTADO_DEUDA, CODREMESA, CUENTA, NRODOC, TIPODOCUMENTO, NOM_TIPO_DOCUMENTO FROM CUOTA, TIPO_DOCUMENTO WHERE RUTDEUDOR='"& rut &"' AND CODCLIENTE='"& strCodCliente &"' AND SALDO > 0 AND ESTADO_DEUDA IN ('1','7','8') AND CUOTA.TIPODOCUMENTO = TIPO_DOCUMENTO.COD_TIPO_DOCUMENTO ORDER BY CUENTA, FECHAVENC DESC"
				...
				If Trim(strEstadoDeuda) = "1"  Then
					...
					If intAntiguedad > 0 Then
						...
						If Trim(intTipoDocHono) = Trim(rsDET("TIPODOCUMENTO")) Then
							...
							intHonorariosDoc = GastosCobranzas(HonorariosEspeciales1(intSaldo,intCMeses))
						Else
							intHonorariosDoc = GastosCobranzas(intSaldo)
						End If
