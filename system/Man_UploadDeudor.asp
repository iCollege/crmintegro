...
If strArchivo <> "" Then
	...
	strSql = strSql &" RUT_REP_LEGAL VARCHAR(12) COLLATE SQL_Latin1_General_CP1_CI_AS NULL, NOM_REP_LEGAL VARCHAR(100) COLLATE SQL_LATIN1_GENERAL_CP1_CI_AS NULL)"
	Conn.Execute strSql,64
	...
	strSql = "INSERT INTO DEUDOR (RUTDEUDOR, NOMBREDEUDOR,FECHA_INGRESO, USUARIO_INGRESO, CODCLIENTE, REPLEG_RUT, REPLEG_NOMBRE, OBS_CARGA)"
	strSql = strSql & " SELECT DISTINCT RUT,NOMBRE,GETDATE(),1, COD_CLIENTE, RUT_REP_LEGAL, NOM_REP_LEGAL, '" & strObsCarga & "'"
	strSql = strSql & " FROM CARGA_DEUDOR WHERE COD_CLIENTE = '" & Request("CB_CLIENTE") & "' AND RUT NOT IN ( "
	strSql = strSql & " SELECT RUTDEUDOR FROM DEUDOR WHERE CODCLIENTE = '" & Request("CB_CLIENTE") & "') "
	'Response.write "strSql = " & strSql
	'Response.eND
	Conn.Execute strSql,64
