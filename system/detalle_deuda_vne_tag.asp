...
<table width="680" height="420" border="0">
  ...
  <tr>
	...
	abrirscg()
	ssql=""
	ssql="SELECT FECHAVENC,NRODOC,VALORCUOTA,SALDO,NSERVICIO,CODCOBRADOR,NROCUOTA FROM CUOTA WHERE RUTDEUDOR='"&rut&"' AND CODCLIENTE='"&cliente&"'"
	'response.Write(ssql)
	'response.End()
	...
	%>
	  <table width="100%" border="0" bordercolor="#FFFFFF">
        ...
        <tr bordercolor="#999999" >
          ...
          <td><div align="right" > <%
		  if not isnull(rsDET("NSERVICIO")) then
		  response.Write(rsDET("NSERVICIO"))
		  else
		  response.Write("N/A")
		  end if
		  %></div></td>
