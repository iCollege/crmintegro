...
If strArchivo <> "" Then
	...
	'**********CARGA ARCHIVO************'
	strSqlFile = "BULK INSERT TMP_DEUDOR_TELEFONOS FROM '" & strFileDir & "' with ( fieldterminator =';',ROWTERMINATOR ='\n', FIRSTROW = 2) "
	Conn.Execute strSqlFile,64
	'response.write "strSqlFile " & strSqlFile

	strSql = "SELECT * FROM TMP_DEUDOR_TELEFONOS"
	...
	Do While not rsTemp.eof
		...
		codarea = rsTemp("CODAREA")
		telefono = rsTemp("TELEFONO")


		strSqlc = "SELECT ISNULL(COUNT(*),0) AS CANTIDAD FROM DEUDOR_TELEFONO WHERE Telefono = '"&telefono&"'"
		set rsTempc= Conn.execute(strSqlc)
		'response.write "strSqlc " & strSqlc
		if rsTempc("CANTIDAD") = 0 then

		ssql="EXEC SCG_WEB_NUEVO_TEL '"&rut&"','"&codarea&"','"&telefono&"','"&session("session_login")&"'"
		Conn.execute(ssql)
	...
	strSqldel = "DELETE FROM TMP_DEUDOR_TELEFONOS"
