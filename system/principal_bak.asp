...
-->
		...
		if rut <> "" and not isnull(rut) then
			...
			if existe = "si" then
				...
				abrirSCG()
					strSql = "SELECT TOP 1 NOMBREDEUDOR,RUTDEUDOR,ISNULL(DEUDOR.REPLEG_RUT,'') as REPLEG_RUT, ISNULL(DEUDOR.REPLEG_NOMBRE,'') as REPLEG_NOMBRE, CAMPANA.NOMBRE, ESTADO_COBRANZA.NOM_ESTADO_COBRANZA,IC,MONTO,DIAS_DE_MORA"
...
<button onclick="telus()"" value="va el rut" type="button" class="btn btn-default btn-xs">BUSCAR CUENTAS </button><br><br>
				...
				<table width="100%" border="1" bordercolor="#FFFFFF">
				<tr bordercolor="#999999"  bgcolor="#<%=session("COLTABBG")%>" class="Estilo13">
					<td >PATENTE</td><td  class="Estilo13">IC</td><td>MONTO ASIGNADO</td><td>DIAS DE MORA</td>
				</tr>
				<tr bgcolor="#<%=session("COLTABBG2")%>" class="Estilo8">
					<td><%=rut_patente%></td>
					<td><%=ic%></td>
