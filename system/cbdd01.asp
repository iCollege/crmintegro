...
ssql="SELECT *, CONVERT(VARCHAR(8),GETDATE(),108) AS HH , CONVERT(VARCHAR(10),GETDATE(),103) AS FH FROM USUARIO WHERE LOGIN='" & login & "' AND CLAVE = '" & clave & "' AND ID_USUARIO IN (SELECT ID_USUARIO FROM USUARIO_CLIENTE WHERE CODCLIENTE = '" & request("CB_CLIENTE") & "')"
...
set rsUSU=Conn.execute(ssql)
	if not rsUSU.eof then
		...
		if ok="Si" then
			...
			strSql="SELECT NOMBRE_CONV_PAGARE,COD_MONEDA FROM CLIENTE WHERE CODCLIENTE = '" & request("CB_CLIENTE") & "'"
