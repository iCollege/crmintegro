  ...
  <tr>
	...
	<table width="100%" border="0" bordercolor="#999999">
		...
		<a href="javascript:showCal('Calendar7');"><img src="../../images/calendario.gif" border="0"></a>
          ...
          <a href="javascript:showCal('Calendar6');"><img src="../../images/calendario.gif" border="0"></a>
			...
			<td>
			<input type="Button" name="Ver" value="Ver" onClick="envia();">
			<input type="Button" name="Cerrar" value="Cerrar" onClick="Ingresa();" DISABLED>
			</td>
	...
	'dim vectormontos
		...
		if not rsDet.eof then
			...
			Do while not rsDet.eof
				...
				strSql = strSql & " WHERE CWC.ID_PAGO = CWDP.ID_PAGO AND CLI.CODCLIENTE = CWC.COD_CLIENTE AND CWC.RUTDEUDOR = DEU.RUTDEUDOR AND CLI.CODCLIENTE = DEU.CODCLIENTE "
	...
	<table width="100%" border="0" bordercolor="#000000">
		...
		DO WHILE X < 2
			...
			strSql = strSql & " WHERE CWC.ID_PAGO = CWDP.ID_PAGO AND CWDP.COD_BANCO *= B.CODIGO AND CLI.CODCLIENTE = CWC.COD_CLIENTE AND CWC.RUTDEUDOR = DEU.RUTDEUDOR AND CLI.CODCLIENTE = DEU.CODCLIENTE "
		...
		if not rsCliente.eof then
			do while not rsCliente.eof
				...
				strSql = "SELECT  ISNULL(SUM(CWDP.MONTO),0) AS MONTO FROM CAJA_WEB_EMP CWC,CAJA_WEB_EMP_DOC_PAGO CWDP, CLIENTE CLI, DEUDOR DEU WHERE CWC.ID_PAGO = CWDP.ID_PAGO AND CLI.CODCLIENTE = CWC.COD_CLIENTE AND CWC.RUTDEUDOR = DEU.RUTDEUDOR AND CLI.CODCLIENTE = DEU.CODCLIENTE "
				If Trim(TraeSiNo(session("perfil_sup"))="No") Then
					strSql=strSql & " AND CWC.USRINGRESO = " & intCodUsuario
				End if
				If Trim(intUsuario) <> "" Then
				...
				strSql = "SELECT  ISNULL(SUM(CWDP.MONTO),0) AS MONTO ,NOMBRE_B FROM CAJA_WEB_EMP CWC,CAJA_WEB_EMP_DOC_PAGO CWDP, BANCOS B, CLIENTE CLI, DEUDOR DEU WHERE CWC.ID_PAGO = CWDP.ID_PAGO AND CWDP.COD_BANCO = B.CODIGO AND CLI.CODCLIENTE = CWC.COD_CLIENTE AND CWC.RUTDEUDOR = DEU.RUTDEUDOR AND CLI.CODCLIENTE = DEU.CODCLIENTE "
				If Trim(TraeSiNo(session("perfil_sup"))="No") Then
					strSql=strSql & " AND CWC.USRINGRESO = " & intCodUsuario
				End if
				If Trim(intUsuario) <> "" Then
		...
		<TR bordercolor="#999999"  bgcolor="#<%=session("COLTABBG")%>" class="Estilo13">
			<TD><b>TOTAL</b></TD>
			<TD ALIGN = "RIGHT" BGCOLOR="#<%=session("COLTABBG2")%>"><b><A HREF="detalle_cuadratura.asp?resp=SI&INICIO=<%=INICIO%>&TERMINO=<%=TERMINO%>&CLIENTE=0&CB_FORMA_PAGO=EF&CB_DESTINO="><%=FORMATNUMBER(intTotalEfectivo,0)%></A></b></TD>
			<TD></TD>
...
<br>
  ...
  <tr>
	...
	<%IF GRABA = "si" THEN
		...
		If Trim(intUsuario) <> "" Then
			strSqlQry="SELECT FECHA_CIERRE FROM CAJA_WEB_EMP_CIERRE WHERE COD_USUARIO = '" & intUsuario & "' AND CONVERT(VARCHAR(10),FECHA_APERTURA,103) = '" & dtmFechaCierre & "'"
			intUsuarioCierre = intUsuario
		Else
			strSqlQry="SELECT FECHA_CIERRE FROM CAJA_WEB_EMP_CIERRE WHERE COD_USUARIO = '" & intCodUsuario & "' AND CONVERT(VARCHAR(10),FECHA_APERTURA,103) = '" & dtmFechaCierre & "'"
		...
		If Not rsCIERRE.EOF  THEN
			...
			If Trim(rsCIERRE("FECHA_CIERRE")) = "" or IsNull(rsCIERRE("FECHA_CIERRE")) then
				...
				strSql = "UPDATE CAJA_WEB_EMP_CIERRE SET FECHA_CIERRE = '" & dtmFechaCierre & "'"
				strSql = strSql & " WHERE COD_USUARIO = " & intUsuarioCierre
				strSql = strSql & " AND CONVERT(VARCHAR(10),FECHA_APERTURA,103) = '" & dtmFechaCierre & "'"
				'Response.write (strsql)
