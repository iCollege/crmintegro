...
<%
	...
	abrirscg()
	strSql=""
	strSql="SELECT MAX(ID_CONVENIO) as ID_CONVENIO FROM CONVENIO_ENC WHERE RUTDEUDOR = '" & strRutDeudor & "' AND COD_CLIENTE = '" & strCodCliente & "'"
	set rsID_CONVENIO=Conn.execute(strSql)
	If Not rsID_CONVENIO.eof Then
		intId_Convenio = rsID_CONVENIO("ID_CONVENIO")
	End if

	'Response.write "intId_Convenio=" & intId_Convenio
	'Response.End

	If Trim(intId_Convenio) = "" or IsNull(intId_Convenio) Then
		%>
	...
	If Trim(intId_Convenio) <> "" Then
		strSql=""
		strSql="SELECT TOTAL_CONVENIO - PIE as TOTAL FROM CONVENIO_ENC WHERE ID_CONVENIO = " & intId_Convenio
		'Response.write "strSql=" & strSql
		'Response.End

		set rsID_CONVENIO=Conn.execute(strSql)
		If Not rsID_CONVENIO.eof Then
			intTotalaPagar = rsID_CONVENIO("TOTAL")
		End if
	Else
		intTotalaPagar = ""
	End if
	...
	If Trim(intId_Convenio) <> "" Then
		strSql=""
		strSql="SELECT SUM(MONTO) AS EFECTIVO FROM CAJA_WEB_EMP_DOC_PAGO WHERE FORMA_PAGO IN ('EF','AB') AND ID_CONVENIO = " & intId_Convenio
		'set rsID_CONVENIO=Conn.execute(strSql)
		'If Not rsID_CONVENIO.eof Then
		'	intTotalEFectivo = rsID_CONVENIO("EFECTIVO")
		'End if
	End If
	...
	If Trim(intId_Convenio) <> "" Then
		...
		strSql=""
		strSql="SELECT COUNT(CUOTA) AS CANTCUOTAS, MAX(TOTAL_CUOTA) AS VALORCUOTA, MAX(FECHA_PAGO) as MAXVENCIMIENTO, MIN(FECHA_PAGO) as MINVENCIMIENTO FROM CONVENIO_DET WHERE CUOTA <> 0 AND ID_CONVENIO = " & intId_Convenio
		set rsID_CONVENIO=Conn.execute(strSql)
		If Not rsID_CONVENIO.eof Then
			intCantCuotas = rsID_CONVENIO("CANTCUOTAS")
			intValorCuota = rsID_CONVENIO("VALORCUOTA")
			intDiaCuota1= Mid(rsID_CONVENIO("MINVENCIMIENTO"),1,2)
			intMesCuota1= Mid(rsID_CONVENIO("MINVENCIMIENTO"),4,2)
			intAnoCuota1= Mid(rsID_CONVENIO("MINVENCIMIENTO"),7,4)
			intDiaCuotaN= Mid(rsID_CONVENIO("MAXVENCIMIENTO"),1,2)
			intMesCuotaN= Mid(rsID_CONVENIO("MAXVENCIMIENTO"),4,2)
			intAnoCuotaN= Mid(rsID_CONVENIO("MAXVENCIMIENTO"),7,4)
		End if
	...
	strSql=""
	strSql="SELECT NRODOC FROM CONVENIO_CUOTA WHERE ID_CONVENIO = " & intId_Convenio
