...
<% ' Capa 2 ' %>
<!--#include file="asp/comunes/select/Procurador.inc"-->
<!--#include file="asp/comunes/delete/Procurador.inc"-->
<%
    AbrirSCG()
        ...
        If intIndiceFilasIni <> "" Then
            ...
            For intRow = intIndiceFilasIni to intIndiceFilasFin
                ...
                If sarrBorrar( 0, intRow ) = "on" Then
                    delete_Procurador Conn, sarrBorrar(1, intRow), strMsg
    ...
    select_Procurador Conn, sarrRegistros, sintTotalRegistros
...
<FORM NAME="mantenedorForm" method="POST" action="man_Procurador.asp" onmouseover="highlightButton('start')" onmouseout="highlightButton('')">
...
<TABLE WIDTH="100%" border=0 cellspacing=0 CLASS="tabla1">
    ...
    <TR BGCOLOR="#FFFFFF" HEIGHT="30" VALIGN="MIDDLE">
        ...
        <INPUT TYPE=BUTTON NAME="Nuevo" VALUE="Nuevo" onClick="IrNuevo( 'man_ProcuradorForm.asp' )">
...
<%
    For intRow = intIndiceFilasIni to intIndiceFilasFin
		...
		If Trim(sarrRegistros(intRow).Item("IDPROCURADOR"))="" or IsNull(sarrRegistros(intRow).Item("IDPROCURADOR")) Then
			intIdProcurador=0
		Else
			intIdProcurador=sarrRegistros(intRow).Item("IDPROCURADOR")
		End if
			'Response.write "<br>intIdProcurador =" & intIdProcurador

%>
<TR BGCOLOR="<%=sstrColor%>">
	<TD>&nbsp;</TD>
    <TD>
        <INPUT TYPE=HIDDEN NAME="sstrId<%= Cstr( intRow ) %>" VALUE="<%= Trim(sarrRegistros(intRow).Item("IDPROCURADOR"))%>">
        <%= Trim(sarrRegistros(intRow).Item("IDPROCURADOR")) %>
    </TD>
    <TD>
		<A HREF="man_PROCURADORForm.asp?sintNuevo=0&IDPROCURADOR=<%=Trim(sarrRegistros(intRow).Item("IDPROCURADOR"))%>">
		<%= Trim(sarrRegistros(intRow).Item("NOMPROCURADOR")) %>
		</A>
    </TD>
    <TD>
...
<SCRIPT>
function Refrescar(){
	mantenedorForm.action='man_Procurador.asp?intFiltro=1';
