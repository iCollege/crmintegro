...
<!--#include file="asp/comunes/delete/EstadoDemanda.inc"-->
<%
    AbrirSCG()
        ...
        If intIndiceFilasIni <> "" Then
            ...
            For intRow = intIndiceFilasIni to intIndiceFilasFin
                ...
                If sarrBorrar( 0, intRow ) = "on" Then
                    delete_EstadoDemanda Conn, sarrBorrar(1, intRow), strMsg
    ...
    select_EstadoDemanda Conn, sarrRegistros, sintTotalRegistros
...
<FORM NAME="mantenedorForm" method="POST" action="man_EstadoDemanda.asp" onmouseover="highlightButton('start')" onmouseout="highlightButton('')">
...
<TABLE WIDTH="100%" border=0 cellspacing=0 CLASS="tabla1">
    ...
    <TR BGCOLOR="#FFFFFF" HEIGHT="30" VALIGN="MIDDLE">
        ...
        <INPUT TYPE=BUTTON NAME="Nuevo" VALUE="Nuevo" onClick="IrNuevo( 'man_EstadoDemandaForm.asp' )">
...
<%
    For intRow = intIndiceFilasIni to intIndiceFilasFin
		...
		If Trim(sarrRegistros(intRow).Item("IDESTADODEMANDA"))="" or IsNull(sarrRegistros(intRow).Item("IDESTADODEMANDA")) Then
			intIdEstadoDemanda=0
		Else
			intIdEstadoDemanda=sarrRegistros(intRow).Item("IDESTADODEMANDA")
		End if
			'Response.write "<br>intIdEstadoDemanda =" & intIdEstadoDemanda

%>
<TR BGCOLOR="<%=sstrColor%>">
	<TD>&nbsp;</TD>
    <TD>
        <INPUT TYPE=HIDDEN NAME="sstrId<%= Cstr( intRow ) %>" VALUE="<%= Trim(sarrRegistros(intRow).Item("IDESTADODEMANDA"))%>">
        <%= Trim(sarrRegistros(intRow).Item("IDESTADODEMANDA")) %>
    </TD>
    <TD>
		<A HREF="man_ESTADODEMANDAForm.asp?sintNuevo=0&IDESTADODEMANDA=<%=Trim(sarrRegistros(intRow).Item("IDESTADODEMANDA"))%>">
		<%= Trim(sarrRegistros(intRow).Item("NOMESTADODEMANDA")) %>
		</A>
    </TD>
    <TD>
...
<SCRIPT>
function Refrescar(){
	mantenedorForm.action='man_EstadoDemanda.asp?intFiltro=1';
